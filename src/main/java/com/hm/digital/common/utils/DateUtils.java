...
//按照减法原理，先day相减，不够向month借；然后month相减，不够向year借；最后year相减。
  ...
  public static String getStartTime(String date) {
    ...
    SimpleDateFormat format = new SimpleDateFormat(DateUtils.DATE_TIME_FORMAT2);
  ...
  public static Date getCurrentQuarterStartTime(Date date) {
    ...
    try {
      ...
      log.error("获取当季开始时间出错", e.fillInStackTrace());
    }
    return c.getTime();
  }
  ...
  public static List<Date> getMonths(Date startDate, Date endDate) {
    ...
    try {
      ...
      log.error("解析失败", e.fillInStackTrace());
    }
  ...
  public static String date2String(Date date) {
    ...
    return new SimpleDateFormat(DateUtils.DATE_TIME_FORMAT2).format(date);
  }
